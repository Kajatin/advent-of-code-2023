...
struct Strength {
    ...
    init(cards: [Card]) {
        ...
        // Correct to five of a kind
        if (letterCounts.keys.contains("J") && letterCounts.keys.count <= 2) || letterCounts.values.contains(5) {
            self.value = 7
            self.name = "Five of a kind"
        } else if letterCounts.filter({ $0.key != "J" }).values.contains(letters.count - letterCounts["J", default: 0] - 1) || letterCounts.values.contains(4) {
            self.value = 6
            self.name = "Four of a kind"
        } else if (letterCounts.keys.count == 3 && letterCounts["J", default: 0] >= 1) || (letterCounts.keys.count == 2 && letterCounts["J", default: 0] >= 2) || (letterCounts.values.contains(3) && letterCounts.values.contains(2)) {
            self.value = 5
            self.name = "Full house"
        } else if letterCounts["J", default: 0] >= 2 || (letterCounts.values.contains(2) && letterCounts["J", default: 0] >= 1) || letterCounts.values.contains(3) {
            self.value = 4
            self.name = "Three of a kind"
        } else if letterCounts.values.filter({ $0 == 2 }).count == 2 || (letterCounts.values.filter({ $0 == 2 }).count == 1 && letterCounts["J", default: 0] >= 1) || letterCounts["J", default: 0] >= 2 {
            self.value = 3
            self.name = "Two pairs"
        } else if letterCounts.values.contains(2) || letterCounts["J", default: 0] >= 1 {
            self.value = 2
            self.name = "One pair"
        } else {
            self.value = 1
...
for (index, hand) in hands.enumerated() {
    ...
    // print("\(index + 1): \(hand.bid) * \(index + 1) = \(hand.bid * (index + 1)) (\(hand.cards.map({ $0.letter }).joined(separator: "")) \(hand.strength.name))")
}
print("Day 7 Puzzle 2: \(summa)")
